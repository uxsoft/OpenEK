...
namespace OpenEK.Windows.Extensions
{
    public class LoessInterpolator
    {
         ...
         */
        private int robustnessIters = DEFAULT_ROBUSTNESS_ITERS;

        public LoessInterpolator(double bandwidth = DEFAULT_BANDWIDTH, int robustnessIters = DEFAULT_ROBUSTNESS_ITERS)
        {
        ...
        public double[] Smooth(double[] xval, double[] yval)
        {
            ...
            for (int iter = 0; iter <= robustnessIters; ++iter)
            {
                ...
                for (int i = 0; i < n; ++i)
                {
                    ...
                    // farthest from x
                    int edge = xval[i] - xval[ileft] > xval[iright] - xval[i] ? ileft : iright;
                    ...
                    double beta = meanXSquared == meanX * meanX ? 0 : (meanXY - (meanX * meanY)) / (meanXSquared - (meanX * meanX));
